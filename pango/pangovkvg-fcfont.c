/* Pango
 * pangovkvgfc-font.c: Vkvg font handling, fontconfig backend
 *
...
struct _PangoVkvgFcFont
{
  PangoFcFont font;
  PangoVkvgFontPrivate cf_priv;
};

struct _PangoVkvgFcFontClass
{
  PangoFcFontClass  parent_class;
};

_PANGO_EXTERN
GType pango_vkvg_fc_font_get_type (void);
...
pango_vkvg_fc_font_create_font_face (PangoVkvgFont *cfont)
{
...
pango_vkvg_fc_font_create_base_metrics_for_context (PangoVkvgFont *cfont,
						     PangoContext   *context)
{
  PangoVkvgFcFont *cffont = (PangoVkvgFcFont *) cfont;
  PangoFcFont *fcfont = (PangoFcFont *) cfont;
  PangoFontMetrics *metrics;

  metrics = pango_fc_font_create_base_metrics_for_context (fcfont, context);

  if (_pango_vkvg_font_private_is_metrics_hinted (&cffont->cf_priv))
    {
...
{
  iface->create_font_face = pango_vkvg_fc_font_create_font_face;
  iface->create_base_metrics_for_context = pango_vkvg_fc_font_create_base_metrics_for_context;
  iface->cf_priv_offset = G_STRUCT_OFFSET (PangoVkvgFcFont, cf_priv);
}
...
{
  PangoVkvgFcFont *cffont = (PangoVkvgFcFont *) object;

  _pango_vkvg_font_private_finalize (&cffont->cf_priv);

  G_OBJECT_CLASS (pango_vkvg_fc_font_parent_class)->finalize (object);
}
...
static void
pango_vkvg_fc_font_get_glyph_extents (PangoFont      *font,
				       PangoGlyph      glyph,
				       PangoRectangle *ink_rect,
				       PangoRectangle *logical_rect)
{
  PangoVkvgFcFont *cffont = (PangoVkvgFcFont *) (font);

  _pango_vkvg_font_private_get_glyph_extents (&cffont->cf_priv,
					       glyph,
					       ink_rect,
					       logical_rect);
}
...
static FT_Face
pango_vkvg_fc_font_lock_face (PangoFcFont *font)
{
  PangoVkvgFcFont *cffont = (PangoVkvgFcFont *) (font);
  vkvg_scaled_font_t *scaled_font = _pango_vkvg_font_private_get_scaled_font (&cffont->cf_priv);
...
static void
pango_vkvg_fc_font_unlock_face (PangoFcFont *font)
{
  PangoVkvgFcFont *cffont = (PangoVkvgFcFont *) (font);
  vkvg_scaled_font_t *scaled_font = _pango_vkvg_font_private_get_scaled_font (&cffont->cf_priv);
...
static void
pango_vkvg_fc_font_class_init (PangoVkvgFcFontClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  PangoFontClass *font_class = PANGO_FONT_CLASS (class);
  PangoFcFontClass *fc_font_class = PANGO_FC_FONT_CLASS (class);

  object_class->finalize = pango_vkvg_fc_font_finalize;

  font_class->get_glyph_extents = pango_vkvg_fc_font_get_glyph_extents;
  font_class->get_metrics = _pango_vkvg_font_get_metrics;

  fc_font_class->lock_face = pango_vkvg_fc_font_lock_face;
  fc_font_class->unlock_face = pango_vkvg_fc_font_unlock_face;
}

static void
pango_vkvg_fc_font_init (PangoVkvgFcFont *cffont G_GNUC_UNUSED)
{
}
...
PangoFcFont *
_pango_vkvg_fc_font_new (PangoVkvgFcFontMap *cffontmap,
			  PangoFcFontKey      *key)
{
  PangoVkvgFcFont *cffont;
  ...
  _pango_vkvg_font_private_initialize (&cffont->cf_priv,
					(PangoVkvgFont *) cffont,
					get_gravity (pattern),
					options,
					pango_fc_font_key_get_matrix (key),
					&font_matrix);

  ((PangoFcFont *)(cffont))->is_hinted = _pango_vkvg_font_private_is_metrics_hinted (&cffont->cf_priv);
